...
global DAQPARS outputData %#ok<GVMIS> 

if isfield(DAQPARS,'outputChannels')
    outputChannelsOld = DAQPARS.outputChannels;
else
    outputChannelsOld = [];
end
...
DAQPARS.outputChannels = find(sum(p.buttons,1));
testChannelsOn = find(MainApp.UITestPulse.Data);
DAQPARS.outputChannels = union(testChannelsOn,DAQPARS.outputChannels);
...
if ~isempty(outputChannelsOld)
    updateBoard = updatenidaqboard([],[],outputChannelsOld,DAQPARS.outputChannels);
    if updateBoard
